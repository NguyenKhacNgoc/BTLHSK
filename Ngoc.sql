...
BEGIN
	...
	UPDATE dbo.tblMatHang SET iSoLuong = iSoLuong + @SoLuong
...
AS 
BEGIN
    DECLARE @MaHD INT, @MaMH INT, @SLB INT, @GB FLOAT, @TGBH INT, @GC NVARCHAR(30), @SLT INT
	SELECT @MaHD = iMaHD, @MaMH = iMaMH, @GB = fGiaBan, @TGBH = iThoiGianBaoHanh, @GC = sGhiChu FROM inserted 
	SELECT @SLB = iSoLuong FROM inserted 
